...
@app.post("/question")
def get_answer(customer_question: CustomerQuestion):
    answer = agent.invoke({"messages": [HumanMessage(content=customer_question.customer_question)]}, config={"configurable": {"thread_id": customer_question.thread_id}})
    return {"answer": answer["messages"][-1].content} # return the last message of the agent